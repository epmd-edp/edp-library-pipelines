...
class Job {
        ...
        if (autodeployLatestVersions == "true") {
            try {
                script.timeout(time: autodeployTimeout, unit: 'MINUTES') {
                        submitter = setCodebaseVersionFromUser()
                        script.println("VVVV2: ${submitter} ")
                }
            } catch (org.jenkinsci.plugins.workflow.steps.FlowInterruptedException ex) {
                    script.println("VVVV: ${submitter} ")
    ...
    private def setCodebaseVersionFromUser() {
        ...
        userInputImagesToDeploy = script.input id: 'userInput', message: 'Provide the following information', parameters: deployJobParameters , submitterParameter: 'submitter'
        script.println("[JENKINS][DEBUG] USERS_INPUT_IMAGES_TO_DEPLOY: ${userInputImagesToDeploy[\"${codebase.name.toUpperCase().replaceAll("-", "_")}_VERSION\"]}")
        script.println("Submitter ${userInputImagesToDeploy.submitter} ")
        codebasesList.each() { codebase ->
        ...
        return userInputImagesToDeploy.submitter
    }
