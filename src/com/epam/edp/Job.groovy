        ...
        if (autodeployLatestVersions) {
            try {
                script.timeout(time: autodeployTimeout, unit: 'MINUTES') {
                    ...
                    codebasesList.each() { codebase ->
                        codebase.latest = "noImageExists" ? "No deploy" : codebase.latest
                        codebase.version = LATEST_TAG
                        script.println("[JENKINS][DEBUG] AUTO_INPUT_IMAGES_TO_DEPLOY: ${codebase.latest}")
