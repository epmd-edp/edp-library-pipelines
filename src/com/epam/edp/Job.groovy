        ...
        if (autodeployLatestVersions == "true") {
            try {
                script.timeout(time: autodeployTimeout, unit: 'MINUTES') {
                    ...
                    codebasesList.each() { codebase ->
                        codebase.latest = codebase.latest == "noImageExists" ? "No deploy" : codebase.latest
                        codebase.version = LATEST_TAG
                        script.println("[JENKINS][DEBUG] AUTO_INPUT_IMAGES_TO_DEPLOY: ${codebase.name.toUpperCase().replaceAll("-", "_")}_VERSION ${codebase.latest}")
                        script.println("Ver: ${codebase.version}")
                    }
            }
        } else {
            getVersionsFromUser()
        }
        script.println(autodeployLatestVersions)
    }

    private def getVersionsFromUser() {
        codebasesList.each() { codebase ->
        ...
        codebasesList.each() { codebase ->
            ...
            script.println(codebase.latest)
