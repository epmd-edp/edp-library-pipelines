...
class GitInfo {
    ...
    def init() {
        ...
        script.println(job.type)
        script.println(JobType.BUILD.getValue())
        switch (job.type) {
            case JobType.CODEREVIEW.getValue():
                ...
                script.println(job.getParameterValue("GERRIT_BRANCH"))
                script.println(job.getParameterValue("ghprbActualCommit"))
                this.branch = job.getParameterValue("GERRIT_BRANCH", job.getParameterValue("ghprbActualCommit"))
            case JobType.BUILD.getValue():
                this.branch = job.getParameterValue("BRANCH")
                script.println(this.branch)
            case [JobType.BUILD.getValue(), JobType.CODEREVIEW.getValue()]:
                script.println(this.branch)
