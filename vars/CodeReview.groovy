    ...
    node(context.codebase.config.jenkinsSlave.toLowerCase()) {
        try {
            ...
            if (script.currentBuild.result == "SUCCESS") {
                ...
                script.sh """
                    kubectl patch codebasebranches.v2.edp.epam.com ${context.codebase.config.name}-${context.git.branch} --type=merge -p '{\"status\": {\"last_success_build\": "${successBuildCount}"}}'
                """
            }
        }
    }
