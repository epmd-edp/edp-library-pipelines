    ...
    node(context.codebase.config.jenkinsSlave.toLowerCase()) {
        try {
            ...
            if (currentBuild.currentResult == 'SUCCESS') {
                ...
                def lastSuccessBuild = context.platform.getJsonPathValue(codebaseBranchesName, "${context.codebase.config.name}-${context.git.branch}", ".spec.build")
                sh """
                    kubectl patch ${codebaseBranchesName} ${context.codebase.config.name}-${context.git.branch} --type=merge -p '{\"spec\": {\"lastSuccessBuild\": "${lastSuccessBuild}"}}'
                """
            }
        }
    }
